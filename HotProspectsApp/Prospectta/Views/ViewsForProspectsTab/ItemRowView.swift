...
struct ItemRow: View {
    ...
    @State var showContactAddedDialog: Bool = true
    
    @Binding var setContactDialog: Bool
   ...
   @State var showContactView: Bool = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showContactView) {
            AddContactView(prospect: prospect, showAddedToContactsDialog: $showContactAddedDialog)
                .environmentObject(eventLocation)
        }
        .onChange(of: showContactAddedDialog) { newVal in
